...
trII_SD = rs*rs/nus /(6.*pi*(1.+q)) * 1./MdoMs
...
Tr =  loadtxt(f1DSCal, usecols=[12])  ## r integrated torque using Sig
dSTr =  loadtxt(f1DSCal, usecols=[17])  ## r integrated torque using deltaSig_phi
...
for i in range(len(r1D)-1):
	DT += (DTdr[i+1] -DTdr[i])*(r1D[i+1] - r1D[i]) ### WE NEED THIS NUMBER AT EACH TIMESTEP THOUGH!
...
vs_simTrq = -(Sig0*dSTr/Ms - a_bin[iS]/(1.+q)*a_bin[iS]/(1.+q)*  (Om_bin[iS]-Om_bin[iS-1])/(t[iS]-t[iS-1]))/(2.*a_bin[iS]/(1.+q) * Om_bin[iS])
chkD =  (Om_bin[iS]-Om_bin[iS-1])/(t[iS]-t[iS-1])
#-------------------------------------------------------------------#
...
aTypeI = abin0 - t*vrI
aTypeII_DD = abin0/(1.+q) -  t*vrII
#aTypeII_LP = abin0*(1. - t/trII_LP86)
aTypeII_SD = abin0/(1.+q)*(1. - t/trII_SD)

rs_SimTrq = abin0/(1.+q) -  tScal*vs_simTrq
a_SimTrq = abin0 - tScal*vs_simTrq

rsTypeII_SD = abin0/(1.+q)*(1. -  t/trII_SD)
###-------------------------------------###
...
##plot(norb, aTypeII_DD, color='green')
#plot(norb, aTypeII_SD, color='red', linestyle='--')
#plot(norb, aTypeII_LP, color='blue')
p1=plot(norb,r_BH1, color='black')
##scatter(norb,r_BH1, marker='x', s=10, color='black')
#plot(norb,r_BH0, color='green')
#p2=scatter(norb_Scal, a_SimTrq, marker='x', s=20, color='red')
p2=plot(norb_Scal, rs_SimTrq, color='red')
xlim(0, max(norb))
