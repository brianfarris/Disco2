...
void LR_speed(double *prim,double r,int * n,double GAMMALAW,double * p_vn,double * p_cf2,double *Fm,double * p_mn,int subtract_w_analytic){
  double P   = prim[PPP];
  double rho = prim[RHO];
  double vr  =   prim[URR];
  double vp;
  if (subtract_w_analytic){
    vp = r*prim[UPP] - w_analytic(r);
  } else{
    vp  = r*prim[UPP];
  }
  double vz  =   prim[UZZ];
...
// this routine is only called by riemann_set_vel.
// It is used to find various L/R quantities. 
void LR_speed_mhd(double *prim,double r,int * n,double ch, double * p_cf2,double *F,double *Fm,double * p_Bn,double * p_B2,int subtract_w_analytic){
  double rho  =   prim[RHO];
  double vr  =   prim[URR];
  double vp;
  if (subtract_w_analytic){
...
void riemann_set_vel(struct Riemann * theRiemann,struct Sim * theSim,double r,double *Bpack,double GAMMALAW,double DIVB_CH,int subtract_w_analytic){
  double L_Mins, L_Plus, L_Star;

  double vnL,cf2L,mnL,BnL,B2L;
  double FL[3], FmL[3];
  LR_speed(theRiemann->primL,r,theRiemann->n,GAMMALAW,&vnL,&cf2L,FmL,&mnL,subtract_w_analytic);
  if (sim_runtype(theSim)==MHD){
    LR_speed_mhd(theRiemann->primL,r,theRiemann->n,DIVB_CH,&cf2L,FL,FmL,&BnL,&B2L,subtract_w_analytic);
  }
  ...
  double FR[3],FmR[3];
  LR_speed(theRiemann->primR,r,theRiemann->n,GAMMALAW,&vnR,&cf2R,FmR,&mnR,subtract_w_analytic);
  if (sim_runtype(theSim)==MHD){
    LR_speed_mhd(theRiemann->primR,r,theRiemann->n,DIVB_CH,&cf2R,FR,FmR,&BnR,&B2R,subtract_w_analytic);
  }
...
void riemann_set_star_hllc_induction(struct Riemann * theRiemann,struct Sim * theSim,double *Bpack,double GAMMALAW){
  double r = theRiemann->r;
  double *prim;
  double Sk;
  double *Uk;
...
void riemann_set_flux_induction(struct Riemann * theRiemann, struct Sim * theSim,double GAMMALAW,double DIVB_CH,int SetState){
  double r = theRiemann->r;
  double *prim;
  double *F;
  if (SetState==LEFT){
...
void riemann_set_B_Psi_face(struct Riemann * theRiemann,struct Sim *theSim){
