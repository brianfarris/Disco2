...
double Gp_r2RhoNu_o_r2RhoNu(double * AvgPrim , double * Grad_ph_prim, double r, double tiph, int IDtype){
  if (VISC_CONST==1){
    return(0.0);
  } else{
    if (IDtype==SHEAR){
...
void riemann_visc_flux(struct Riemann * theRiemann,struct Sim * theSim ){
  ...
  for (q=0;q<NUM_Q;++q){
    ...
    //} else{
    Grad_r_prim[q] = .5*(cell_gradr(theRiemann->cL,q)+cell_gradr(theRiemann->cR,q));
    //}
    VFlux[q] = 0.0;
  }
...
void riemann_visc_flux_old(struct Riemann * theRiemann,struct Sim * theSim ){
  ...
  for (q=0;q<NUM_Q;++q){
    ...
    if (theRiemann->n[1]==1){ 
      ...
      Gprim[q] = .5*(cell_gradr(theRiemann->cL,q)+cell_gradr(theRiemann->cR,q));
    }
    VFlux[q] = 0.0;
  }
...
void riemann_setup_rz(struct Riemann * theRiemann,struct Face * theFaces,struct Sim * theSim,int FaceNumber,int direction){
  ...
  if (direction==R_DIR){
    ...
    printf("SHOULD BE R OR Z DIRECTION\n");
    exit(1);
  }
 ...
 // printf("%e %e\n",sin(pL)*theRiemann->primL[BRR]+cos(pL)*theRiemann->primL[BPP], sin(pR)*theRiemann->primR[BRR]+cos(pR)*theRiemann->primR[BPP]);
}
