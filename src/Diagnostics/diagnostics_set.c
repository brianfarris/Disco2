      ...
      for (i=imin;i<imax;++i){
        ...
        for (j=0;j<sim_N_p(theSim,i);++j){
          ...
          if ((fabs(zp)<0.0000001)||(fabs(z)<0.0000001)){
            ...
            EquatDiag_temp[(theDiagnostics->offset_eq+position)*NUM_EQ+8] = Br;
            EquatDiag_temp[(theDiagnostics->offset_eq+position)*NUM_EQ+9] = Bp;            
          ...
          VectorDiag_temp[(sim_N0(theSim,R_DIR)+i-imin)*NUM_VEC+7] += (Br/sim_N_p(theSim,i)*dz) ;
          VectorDiag_temp[(sim_N0(theSim,R_DIR)+i-imin)*NUM_VEC+8] += (Bp/sim_N_p(theSim,i)*dz) ;
