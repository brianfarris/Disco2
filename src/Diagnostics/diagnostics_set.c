        ...
        for (k=kmin;k<kmax;++k){
            ...
            for (i=imin;i<imax;++i){
                ...
                for (j=0;j<sim_N_p(theSim,i);++j){
                    ...
                    double phi = cell_tiph(c) - 0.5*dphi;
                    double rho = cell_prim(c,RHO);
                    double press = cell_prim(c,PPP);
                    double vr = cell_prim(c,URR);
                    double vp_minus_w = cell_prim(c,UPP)*r;
                    double vp = cell_prim(c,UPP)*r+sim_rOm_a(theSim,r,a);
                    double vz = cell_prim(c,UZZ);
                    ...
                    double Pgas = cell_prim(c,PGAS);
                    double Prad = cell_prim(c,PRAD);
                    ...
                    if ((fabs(zp)<0.0000001)||(fabs(z)<0.0000001)){
                        ...
                        EquatDiag_temp[(theDiagnostics->offset_eq+position)*NUM_EQ+13] = Pgas;
                        EquatDiag_temp[(theDiagnostics->offset_eq+position)*NUM_EQ+14] = Prad;
                    ...
                    VectorDiag_temp[(sim_N0(theSim,R_DIR)+i-imin)*NUM_VEC+12] += (Pgas/sim_N_p(theSim,i)*dz) ;
                    VectorDiag_temp[(sim_N0(theSim,R_DIR)+i-imin)*NUM_VEC+13] += (Prad/sim_N_p(theSim,i)*dz) ;
