			...
			for (i=imin;i<imax;++i){
				...
				for (j=0;j<sim_N_p(theSim,i);++j){
					...
					if ((fabs(zp)<0.0000001)||(fabs(z)<0.0000001)){
						...
						EquatDiag_temp[(theDiagnostics->offset_eq+position)*NUM_EQ+13] = Br;
						EquatDiag_temp[(theDiagnostics->offset_eq+position)*NUM_EQ+14] = Bp;
						EquatDiag_temp[(theDiagnostics->offset_eq+position)*NUM_EQ+15] = Bz;
						EquatDiag_temp[(theDiagnostics->offset_eq+position)*NUM_EQ+16] = B2;
					...
					VectorDiag_temp[(sim_N0(theSim,R_DIR)+i-imin)*NUM_VEC+12] += (Br/sim_N_p(theSim,i)*dz) ;
					VectorDiag_temp[(sim_N0(theSim,R_DIR)+i-imin)*NUM_VEC+13] += (Bp/sim_N_p(theSim,i)*dz) ;
					VectorDiag_temp[(sim_N0(theSim,R_DIR)+i-imin)*NUM_VEC+14] += (Bz/sim_N_p(theSim,i)*dz) ;
					VectorDiag_temp[(sim_N0(theSim,R_DIR)+i-imin)*NUM_VEC+15] += (B2/sim_N_p(theSim,i)*dz) ;
