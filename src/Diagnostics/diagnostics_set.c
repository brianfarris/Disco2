...
void diagnostics_set(struct Diagnostics * theDiagnostics,struct Cell *** theCells,struct Grid * theGrid,struct TimeStep * theTimeStep){
  ...
  double * VectorDiag_temp = malloc(sizeof(double) * num_r_points_global*NUM_DIAG);
  double * VectorDiag_reduce = malloc(sizeof(double) * num_r_points_global*NUM_DIAG);
  ...
  for (i=0;i<num_r_points_global;++i){
    for (n=0;n<NUM_DIAG;++n){
      VectorDiag_temp[i*NUM_DIAG+n]=0.0;
    }
  }
  ...
  for (k=kmin;k<kmax;++k){
    ...
    for (i=imin;i<imax;++i){
      ...
      for (j=0;j<grid_N_p(theGrid,i);++j){
        ...
        VectorDiag_temp[(grid_N_r_0(theGrid)+i-imin)*NUM_DIAG+0] += (cell_prim(cell_single(theCells,i,j,k),RHO)/grid_N_p(theGrid,i)*dz) ;
        VectorDiag_temp[(grid_N_r_0(theGrid)+i-imin)*NUM_DIAG+1] += (cell_prim(cell_single(theCells,i,j,k),PPP)/grid_N_p(theGrid,i)*dz) ;
        //etc
      }
      for (n=0;n<NUM_DIAG;++n){
        // mult by delta r^2 because we are doing an r integration
        ScalarDiag_temp[n] += VectorDiag_temp[(grid_N_r_0(theGrid)+i-imin)*NUM_DIAG+n] * (rp*rp-rm*rm); 
      }
    }
  }

  MPI_Allreduce( ScalarDiag_temp,ScalarDiag_reduce, NUM_DIAG, MPI_DOUBLE, MPI_SUM, grid_comm);
  MPI_Allreduce( VectorDiag_temp,VectorDiag_reduce, num_r_points_global*NUM_DIAG, MPI_DOUBLE, MPI_SUM, grid_comm);
  ...
  for (i=0;i<num_r_points_global;++i){
    for (n=0;n<NUM_DIAG;++n){
      VectorDiag_reduce[i*NUM_DIAG+n] /=(ZMAX-ZMIN);
    }
  }
  ...
  for (i=0;i<num_r_points_global;++i){
    for (n=0;n<NUM_DIAG;++n){
      theDiagnostics->VectorDiag[i][n] += VectorDiag_reduce[i*NUM_DIAG+n] * dtout;
    }
  }
  for (n=0;n<NUM_DIAG;++n){
