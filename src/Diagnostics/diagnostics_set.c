...
void diagnostics_set(struct Diagnostics * theDiagnostics,struct Cell *** theCells,struct Sim * theSim,struct TimeStep * theTimeStep,struct MPIsetup * theMPIsetup,struct GravMass * theGravMasses){
  ...
  //Make a File that only writes 2-afew times  to output constant parameters    
    ...
    if(mpisetup_MyProc(theMPIsetup)==0){
      ...
      fprintf(CstParamFile,"%e %e %e %e %e %e %e %e %e %e %i %e %e %e %e %e %e \n", q, sep0, alpha, Mach0, MdoMs, rmin, rmax, tmigon, tacc, eps, nu_cst, Rcut, Gam, tramp, vRate, Rsink0, Rsink1, Np);
      fclose(CstParamFile);
    }
  }
  iGlob += 1;
 ...
 if (timestep_get_t(theTimeStep)>diagnostics_tdiag_measure(theDiagnostics)){
    ...
    double Pow_temp   = 0.0; // sum total trq each measure
    double Pow_reduce = 0.0;
    ...
    */
    ...
    for (k=kmin;k<kmax;++k){
	  ...
	  //double passive_scalar2;
	  //double passive_scalar3;
	  //if (sim_NUM_Q(theSim) > sim_NUM_C(theSim)){
	  //  passive_scalar   = cell_prim(cell_single(theCells,i,j,k),5);
	  //}
	  
          //if (sim_NUM_Q(theSim) > (sim_NUM_C(theSim) + 1) ){
	  //  passive_scalar   = cell_prim(cell_single(theCells,i,j,k),5);
	  //  passive_scalar2  = cell_prim(cell_single(theCells,i,j,k),6);
	  //}

          //if (sim_NUM_Q(theSim) > (sim_NUM_C(theSim) + 2) ){
	  double passive_scalar   = cell_prim(cell_single(theCells,i,j,k),5);
	  double passive_scalar2  = cell_prim(cell_single(theCells,i,j,k),6);
	  double passive_scalar3  = cell_prim(cell_single(theCells,i,j,k),7);
          //}
	  ...
	  //gravMassForce( theGravMasses , theSim , 1 , r , phi , &fr1 , &fp1 );
	      ...
	      Pow_temp      -= rho* (vp*fBin_phi + vr*Fr)* r*dphi*(rp-rm); // E/s change of bin
            }
	  }	  	    
	    ...
	    EquatDiag_temp[(theDiagnostics->offset_eq+position)*NUM_EQ+20] = passive_scalar2;
	    EquatDiag_temp[(theDiagnostics->offset_eq+position)*NUM_EQ+21] = passive_scalar3;
            ++position;
          }
	  ...
	  if (r > 1.){
	    ...
	    VectorDiag_temp[(sim_N0(theSim,R_DIR)+i-imin)*NUM_VEC+23] += 0.0;
	    VectorDiag_temp[(sim_N0(theSim,R_DIR)+i-imin)*NUM_VEC+24] += 0.0;
	    VectorDiag_temp[(sim_N0(theSim,R_DIR)+i-imin)*NUM_VEC+25] += 0.0;
	  }
	  ...
	  //Define eccentricity and ellipticity measuremnets via MM08                                                                                                       
	  e1a = VectorDiag_temp[(sim_N0(theSim,R_DIR)+i-imin)*NUM_VEC+12];
	  e1b = VectorDiag_temp[(sim_N0(theSim,R_DIR)+i-imin)*NUM_VEC+13];
	  e2a = VectorDiag_temp[(sim_N0(theSim,R_DIR)+i-imin)*NUM_VEC+23];
	  e2b = VectorDiag_temp[(sim_N0(theSim,R_DIR)+i-imin)*NUM_VEC+24];
	  edn = VectorDiag_temp[(sim_N0(theSim,R_DIR)+i-imin)*NUM_VEC+25];
	  VectorDiag_temp[(sim_N0(theSim,R_DIR)+i-imin)*NUM_VEC+26] = sqrt(e1a*e1a + e1b*e1b)/edn;
	  VectorDiag_temp[(sim_N0(theSim,R_DIR)+i-imin)*NUM_VEC+27] = sqrt(e2a*e2a + e2b*e2b)/edn;
    ...
    MPI_Allreduce( &Pow_temp, &Pow_reduce , 1, MPI_DOUBLE, MPI_SUM, sim_comm); //DD
    ...
    double e_1 = ScalarDiag_reduce[25];
    double e_2 = ScalarDiag_reduce[26];
    ...
    if(mpisetup_MyProc(theMPIsetup)==0){
      ...
      fprintf(DiagBpFile,"%e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e %e \n",t, r_bh0, r_bh1, a_bin, phi_bh0, phi_bh1, ecc, E, Ltot, Om, Om, Om, Pow_reduce, gravMass_total_torque(theGravMasses,0), TrScal_reduce, Fr_onBin, Fp_onBin, e_1, e_2);
      fclose(DiagBpFile);
    }
