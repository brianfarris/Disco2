  ...
  for( k=kmin ; k<kmax ; ++k ){
    ...
    for( i=imin ; i<imax ; ++i ){
      ...
      for( j=0 ; j<sim_N_p(theSim,i) ; ++j ){
        ...
        //printf("BRR: %e, BPP: %e, BZZ: %e, dAp_dr: %e, dAr_dp: %e, theCells[k][i][j].prim[APP]: %e, r: %e\n",theCells[k][i][j].prim[BRR],theCells[k][i][j].prim[BPP],theCells[k][i][j].prim[BZZ],dAp_dr,dAr_dp,theCells[k][i][j].prim[APP],r);
      }
