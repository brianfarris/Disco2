...
void cell_single_init_psigrad(struct Cell *theCell, struct Sim *theSim,int i,int j,int k){

  printf("ERROR\n");
  exit(0);
}

void cell_init_psigrad(struct Cell ***theCells,struct Sim *theSim,struct MPIsetup * theMPIsetup) {
  ...
  for (k = 0; k < sim_N(theSim,Z_DIR); k++) {
    for (i = 0; i < sim_N(theSim,R_DIR); i++) {
      ...
      for (j = 0; j < sim_N_p(theSim,i); j++) {
        ...
        theCells[k][i][j].prim[PSI] = 1./(0.1+r);
