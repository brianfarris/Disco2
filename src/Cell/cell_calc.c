...
//Calculate cons from primitives for all cells.
...
//Calculate prims from conservatives for all cells.
    ...
    for(k = 0; k < sim_N(theSim,Z_DIR); k++)
    {
        ...
        for(i = 0; i < sim_N(theSim,R_DIR); i++)
        {
            ...
            for(j = 0; j < sim_N_p(theSim,i); j++)
            {
                ...
                double dV = 0.5*(rm+rp)*(rp-rm)*c->dphi*dz;
                pos[P_DIR] = c->tiph - 0.5*c->dphi;
