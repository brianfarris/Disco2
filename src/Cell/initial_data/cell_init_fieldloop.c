...
void cell_single_init_fieldloop(struct Cell *theCell, struct Sim *theSim,struct GravMass * theGravMasses,int i,int j,int k){
  ...
  theCell->prim[UPP] = 0.0;//omega-Vx*sin(phi);
...
void cell_init_fieldloop(struct Cell ***theCells,struct Sim *theSim,struct GravMass * theGravMasses,struct MPIsetup * theMPIsetup) {
  ...
  for (k = 0; k < sim_N(theSim,Z_DIR); k++) {
    for (i = 0; i < sim_N(theSim,R_DIR); i++) {
      ...
      for (j = 0; j < sim_N_p(theSim,i); j++) {
        ...
        theCells[k][i][j].prim[UPP] = 0.0;//omega-Vx*sin(phi);
