...
void cell_single_init_SSCoolTest(struct Cell *theCell, struct Sim *theSim,int i,int j,int k){
	...
	double M1 = 0.0;//Mtotal/(1.+1./massratio);
	double r0 = M1/Mtotal*sep;
	double r1 = M0/Mtotal*sep;

	//double Mach   = sim_Mach(theSim) *pow(sep,(-0.5)) ; //For uniform cs
	...
	double O2 = omega*omega - OmegaK*OmegaK/Mach/Mach/Gam; //*pow(rho,(Gam-1.)); // add 1/(sigma r) * dP/dr term
	omega = sqrt(O2);
	...
	double Pp = cs*cs*rho/Gam; //pow(rho,Gam);  ///Gam; // 1/gam if isotherm (also change in omega)
...
void cell_init_SSCoolTest(struct Cell ***theCells,struct Sim *theSim,struct MPIsetup * theMPIsetup) {
...
//        }		  
	...
	double Pp = cs*cs*rho/Gam; //pow(rho,Gam);   // 1/gam if isotherm (also change in omega)
