...
double sim_rOm_a(struct Sim * theSim,double r,double a){
  if (sim_W_ANALYTIC_TYPE(theSim)==A_FIXED){
      //no w_analytic
    return(0.0);
  } else if (sim_W_ANALYTIC_TYPE(theSim)==A_KEPLER){
    //keplerian rotation
    return(pow(r,-0.5));
  ...
  } else if (sim_W_ANALYTIC_TYPE(theSim)==A_POWERLAW){
    //Useful for a torus
    return(pow(r,-1.0));
  } else{
    printf("ERROR\n");
...
double sim_dtOm_a(struct Sim * theSim,double r,double a){
  if (sim_W_ANALYTIC_TYPE(theSim)==A_MILOS){
    double a0 = 1.0;
