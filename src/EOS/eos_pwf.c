...
{
    double rho = prim[RHO];
    double P = eos_ppp_pwf(prim, theSim);
    double e = eos_eps_pwf(prim, theSim);
    double dedr = eos_depsdrho_pwf(prim, theSim);
    double dedt = eos_depsdttt_pwf(prim, theSim);
    double dpdr = eos_dpppdrho_pwf(prim, theSim);
    double dpdt = eos_dpppdttt_pwf(prim, theSim);
...
double eos_dpppdrho_pwf(double *prim, struct Sim *theSim)
{
    ...
    double T = prim[TTT] * eos_mp*eos_c*eos_c;  //ergs
    double xnuc = eos_xnuc_pwf(rho, T/eos_k);
    double dxdr = eos_dxnudrho_pwf(rho, T/eos_k);
...
double eos_dpppdttt_pwf(double *prim, struct Sim *theSim)
{
    ...
    double T = prim[TTT] * eos_mp*eos_c*eos_c;  // (ergs)
    double xnuc = eos_xnuc_pwf(rho, T/eos_k);
    double dxdt = eos_dxnudttt_pwf(rho, T/eos_k)/eos_k; // (#/ergs)
...
double eos_depsdrho_pwf(double *prim, struct Sim *theSim)
{
    ...
    double T = prim[TTT] * eos_mp*eos_c*eos_c;  //ergs
    double xnuc = eos_xnuc_pwf(rho, T/eos_k);
    double dxdr = eos_dxnudrho_pwf(rho, T/eos_k); // (cm^3/g)
...
double eos_depsdttt_pwf(double *prim, struct Sim *theSim)
{
    ...
    double T = prim[TTT] * eos_mp*eos_c*eos_c;  //ergs
    double xnuc = eos_xnuc_pwf(rho, T/eos_k);
    double dxdt = eos_dxnudttt_pwf(rho, T/eos_k)/eos_k; // (#/erg)
...
double eos_dxnudrho_pwf(double rhocgs, double Tk)
{
    //Returns d(X_nuc)/dRho in CGS units.

    double xnuc = eos_xnuc_pwf(rhocgs, Tk);
    if(xnuc > 1.0)
        return 0.0;
...
double eos_dxnudttt_pwf(double rhocgs, double Tk)
{
